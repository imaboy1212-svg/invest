...
3. 증권 뉴스 크롤링 (네이버증권 주요뉴스 우선, 실패 시 한국경제/매일경제/서울경제/파이낸셜뉴스로 폴백. 소스는 필요에 따라 계속 추가 가능)
3-1. 종목리포트 후보 종목 발굴 (하드코딩 리스트 없음) — 네이버증권 코스피·코스닥 인기종목마다 개별 종목 뉴스·공시를 직접 조회해서, 실제 뉴스·공시가 있는 종목만 후보로 선정 (없으면 후보에서 제외, 억지로 만들지 않음)
3-2. IPO 청약·상장 일정 조회 (네이버증권 공모주 페이지) — 실제 예정 일정 기반으로만 IPO 주제 작성
4. Gemini API로 팀별 주제 후보 3건 + 상세 브리핑 생성 (종목리포트는 발굴된 후보 종목을, IPO는 조회된 일정을 우선 활용)
