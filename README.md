...
3-1. 종목리포트 후보 종목 발굴 (하드코딩 리스트 없음) — 네이버증권 코스피·코스닥 인기종목 중 실제 뉴스가 확인된 종목만 후보로 선정 (뉴스 없으면 후보에서 제외, 억지로 만들지 않음)
4. Gemini API로 팀별 주제 후보 3건 + 상세 브리핑 생성 (종목리포트는 발굴된 후보 종목을 우선 활용)
