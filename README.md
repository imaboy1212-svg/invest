...
**52주 최고가 대비 20~50% 하락**(20% 미만은 "저평가"로 보기 애매해서 제외, 50%
이상은 반등보다 구조적 부실 가능성에 무게를 둬 오히려 제외) 구간에 있으면서
**기술적 지표(RSI 과매도, 골든크로스, MACD, 볼린저밴드 하단 이탈 등 강세 신호)가
하나 이상 확인된** 종목을 골라, 직전 분기 실적(참고용, 필수조건 아님)·최근 공시
(DART)·뉴스(네이버)까지 묶어 텔레그램으로 전송한다.

2026-07-28 최초 버전은 "-50% 이상 하락 + 실적 개선(YoY)"을 둘 다 필수(AND)로
요구해 거의 항상 0건이었다. 사용자 피드백으로 위 기준(하락폭 구간 + 기술적 지표
중심, 실적은 참고정보)으로 재설계함.
...
- `lib/technical_indicators.py` — Yahoo Finance 가격 시계열로 이동평균/골든·
  ...
  (bearish)/중립(neutral) 태그를 붙여 `has_rebound_signal`로 필터링에 씀
- `lib/dart_client.py` — DART Open API로 종목코드→고유번호 매핑(로컬 캐시
  `dart_corp_codes.json`, git 미추적), 최근 공시, 직전 분기 실적(참고용) 조회
- `stock_screener.py` — 위를 엮어 필터링(가격 구간 → 기술적 지표) → 실적/공시/
  뉴스로 정보 보강 → 마크다운 리포트 생성 → 텔레그램 전송
