...
단, 아래 [종목리포트 후보 종목] 목록이 제공된 경우 "수치·뉴스 부족"을 이유로 종목리포트를
비우는 것은 허용되지 않는다 — 그 목록에 있다는 것 자체가 이미 뉴스·공시가 확인됐다는
뜻이므로, 반드시 그 중 하나로 종목리포트를 만들어라 (아래 해당 지시 참고).
...
def _build_prompt(
    ...
    if stock_candidate_lines:
        ...
        context_lines.append(
            "위 후보 종목은 이미 각 종목마다 실제 뉴스·공시가 있음이 확인된 목록이다. "
            "이 목록이 비어있지 않은 이상 종목리포트 주제를 반드시 1건 만들어야 한다 — "
            "후보가 있는데도 '수치·뉴스 부족'을 이유로 종목리포트를 비우는 것은 금지된다. "
            "후보 종목 중 뉴스·공시 근거가 가장 풍부하고 구체적인 종목 하나를 골라, 아래에 "
            "함께 제공된 그 종목의 개별 뉴스 헤드라인을 원문 그대로 인용해 related_news를 "
            "채워라. 후보 목록에 없어도 증권 뉴스 헤드라인에 특정 종목에 대한 확실한 근거가 "
            "있다면 그 종목을 대신 사용해도 된다."
        )
